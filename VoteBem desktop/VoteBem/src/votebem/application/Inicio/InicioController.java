          ...
          if( usu != null && !"".equals(usu)){ 
           ...
           fecharAplicacao();
                try {
                    ...
                    String nomeUsu = conn.findFieldValue("nomeUsu", "Usuario", "idUsuario = '"+usu+"'");
                    //Substring iniciando em 0 até posição do caracter especial para pegar apenas o primeiro nome;
                    int pos = nomeUsu.indexOf(" ");
                    PrincipalController.CurrentController.setLblNomeUser(nomeUsu.substring(0, pos));
                    PrincipalController.CurrentController.setLblNome(nomeUsu);
                    if(usu != null && !"".equals(usu))
                    PrincipalController.CurrentController.setIdUsuario(Integer.parseInt(usu));
                } catch (Exception ex) {
